...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Batch evaluation for SPT vs LLM vs LLM+RAG on JSSP.")
    ...
    parser.add_argument(
        "--error-columns",
        type=str,
        default="llm_error,llm_rag_error",
    ...
    parser.add_argument("--results-dir", type=Path, default=Path("DT") / "results" / "JSSP_BATCH_EVAL")
    parser.add_argument("--num-repeats", type=int, default=1)
    parser.add_argument(
        "--run-modes",
        type=str,
        default="spt,llm,llm_rag",
        help="Comma-separated subset of modes to run: spt,llm,llm_rag",
    )
    parser.add_argument(
        "--runs-csv",
        type=Path,
        default=Path("DT") / "results" / "JSSP_BATCH_EVAL" / "runs.csv",
    )
    parser.add_argument(
        "--summary-csv",
        type=Path,
        default=Path("DT") / "results" / "JSSP_BATCH_EVAL" / "summary.csv",
    )
    ...
    parser.add_argument("--api-url", type=str, default=os.getenv("LLM_API_URL", "https://api.openai.com/v1/chat/completions"))
    ...
    parser.add_argument("--rag-explanatory-cases", action=argparse.BooleanOptionalAction, default=True)
    parser.add_argument("--rag-prefilter-candidate-gap", type=int, default=1)
    parser.add_argument("--rag-conditional", action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument("--rag-ambiguity-threshold", type=float, default=0.12)
    parser.add_argument("--rag-bottleneck-threshold", type=float, default=3.0)
...
def parse_run_modes(value: str) -> list[str]:
    valid_modes = ["spt", "llm", "llm_rag"]
    requested = [item.strip().lower() for item in (value or "").split(",") if item.strip()]
    if not requested:
        raise SystemExit("--run-modes must include at least one of: spt,llm,llm_rag")
    invalid = [item for item in requested if item not in valid_modes]
    if invalid:
        raise SystemExit(f"Invalid --run-modes value(s): {','.join(invalid)}")
    return [mode for mode in valid_modes if mode in requested]
...
def build_llm_agent(
    ...
    base_agent = OpenAISchedulingAgent(
        ...
        rag_explanatory_cases=args.rag_explanatory_cases,
        rag_prefilter_candidate_gap=args.rag_prefilter_candidate_gap,
        rag_conditional=args.rag_conditional,
        rag_ambiguity_threshold=args.rag_ambiguity_threshold,
        rag_bottleneck_threshold=args.rag_bottleneck_threshold,
    )
...
def run_case(problem_data, args: argparse.Namespace, mode: str, event_log_path: Path, decision_log_path: Path, prompt_log_path: Path):
...
def safe_run_case(problem_data, args: argparse.Namespace, mode: str, event_log_path: Path, decision_log_path: Path, prompt_log_path: Path):
    try:
...
def aggregate_runs(df: pd.DataFrame) -> pd.DataFrame:
    ...
    method_cols = [
        ...
        "llm_rag_makespan",
        "delta_llm_vs_spt",
        "delta_llm_rag_vs_spt",
        "delta_llm_rag_vs_llm",
        "llm_prompt_tokens",
        "llm_completion_tokens",
        "llm_total_tokens",
        "llm_usage_records",
        "llm_rag_prompt_tokens",
        "llm_rag_completion_tokens",
        "llm_rag_total_tokens",
        "llm_rag_usage_records",
    ]
    error_cols = ["spt_error", "llm_error", "llm_rag_error"]
    ...
    for instance, g in df.groupby("instance", sort=True):
        ...
        rag_vs_llm = pd.to_numeric(g["delta_llm_rag_vs_llm"], errors="coerce").dropna()
        row["rag_better_than_llm_rate"] = float((rag_vs_llm < 0).mean()) if not rag_vs_llm.empty else float("nan")
        rows.append(row)
...
def main() -> None:
    ...
    if any(mode in {"llm", "llm_rag"} for mode in run_modes) and not args.api_key:
    ...
    for instance in instances:
        ...
        for repeat_idx in range(1, args.num_repeats + 1):
            ...
            rag_log = args.results_dir / f"{suffix}_llm_rag.csv"
            rag_decision = args.results_dir / f"{suffix}_llm_rag_decision.jsonl"
            rag_prompt = args.results_dir / f"{suffix}_llm_rag_prompt.jsonl"

            spt_makespan, spt_error = (float("nan"), "")
            llm_makespan, llm_error = (float("nan"), "")
            rag_makespan, rag_error = (float("nan"), "")
            ...
            if "llm_rag" in run_modes:
                rag_makespan, rag_error = safe_run_case(problem_data, args, "llm_rag", rag_log, rag_decision, rag_prompt)

            llm_usage = token_usage_from_decision_log(llm_decision) if "llm" in run_modes else {
                "prompt_tokens": float("nan"),
                "completion_tokens": float("nan"),
                "total_tokens": float("nan"),
                "usage_records": float("nan"),
            }
            rag_usage = token_usage_from_decision_log(rag_decision) if "llm_rag" in run_modes else {
                "prompt_tokens": float("nan"),
                "completion_tokens": float("nan"),
                "total_tokens": float("nan"),
                "usage_records": float("nan"),
            ...
            row = {
                ...
                "delta_llm_rag_vs_spt": safe_delta(rag_makespan, spt_makespan),
                "delta_llm_rag_vs_llm": safe_delta(rag_makespan, llm_makespan),
                ...
                "llm_rag_prompt_tokens": rag_usage["prompt_tokens"],
                "llm_rag_completion_tokens": rag_usage["completion_tokens"],
                "llm_rag_total_tokens": rag_usage["total_tokens"],
                "llm_rag_usage_records": rag_usage["usage_records"],
                "spt_error": spt_error,
                "llm_error": llm_error,
                "llm_rag_error": rag_error,
                "spt_log": str(spt_log),
                "llm_log": str(llm_log),
            ...
            status_parts = []
            if "spt" in run_modes:
                status_parts.append(f"SPT={spt_makespan:.3f}")
            if "llm" in run_modes:
                status_parts.append(f"LLM={llm_makespan:.3f}")
            if "llm_rag" in run_modes:
                status_parts.append(f"LLM+RAG={rag_makespan:.3f}")
            print(f"    {' | '.join(status_parts)}")

            token_parts = []
            if "llm" in run_modes:
                token_parts.append(f"LLM={llm_usage['total_tokens']:.0f}")
            if "llm_rag" in run_modes:
                token_parts.append(f"RAG={rag_usage['total_tokens']:.0f}")
            if token_parts:
                print(f"      tokens {' | '.join(token_parts)}")
            if llm_error:
                print(f"      LLM error: {llm_error}")
            if rag_error:
                print(f"      LLM+RAG error: {rag_error}")
        ...
        print(
            summary_df[
                [
                    ...
                    "delta_llm_rag_vs_spt_mean",
                    "delta_llm_rag_vs_llm_mean",
                ]
